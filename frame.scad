...
module frame() {
    ...
    difference() {
        ...
        union() {
            ...
            // DIVIDERS
            ring(gap=wall, shell=ledDistance-nametagDepth-.5, onlyPosts=true, sideSize=sideSize-nametagDepth*2-.5);
            
            // BACKWALLS
            ring(gap=0, shell=wall,  sideSize=sideSize-ledDistance*2+wall*2);
        }
        translate([0,0,height/2-wall-stripSize/2])
        linear_extrude(stripSize) // LED Strip Cutout
        ring(gap=0, shell=6,  sideSize=sideSize-ledDistance*2+wall*2+6*2-.5);
        
        translate([0,0,height/2-clipHeight/2])
        linear_extrude(height) // connector spot
...
module clips() {
        ...
        translate([0,0,height/2-clipHeight/2])
        linear_extrude(height/2+clipHeight/2) // connector base
        ring(gap=19, sideSize=sideSize-nametagDepth*2-wall*1.501-.25, shell=2.5, onlyPosts=true);
    
    
        translate([0,0,height/2-clipHeight/2])
        linear_extrude(height/2) // connector spot
        difference() {
